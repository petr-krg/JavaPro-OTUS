package krg.petr.otusru.core.repository;
...
public class DataTemplateHibernate<T> implements DataTemplate<T> {
    ...
    public Optional<T> findByLogin(Session session, String login) {
        return Optional.ofNullable(session.createQuery(String.format("from %s where login = :login",
                clazz.getSimpleName()), clazz).setParameter("login", login).getSingleResult());
    }
}
